...
Before make the source,you must setup CentOS-SCLo-rh.repo, a reference: https://blog.csdn.net/HD2killers/article/details/82728775. 
Then you must create base image directory, using command: 
+
----
cd origin 
s2i create centos7 images/base
s2i create centos7 images/hypershift
s2i create centos7 images/template-service-broker
s2i create centos7 images/hyperkube
s2i create centos7 images/sdn
s2i create centos7 images/cli
s2i create centos7 images/recycler
s2i create centos7 images/deployer
s2i create centos7 images/tests
----

It's recommanded to update golang, a reference:https://tecadmin.net/install-go-on-centos/.
